...
export default function JointVerificationPage() {
  ...
  async function acknowledge() { try { await acknowledgeInspection(currentInspection.id, currentUser.id); showToast('Your acknowledgement was recorded.') } catch (acknowledgeError) { showToast(acknowledgeError instanceof Error ? acknowledgeError.message : 'Acknowledgement could not be recorded.', 'error') } }
  async function lock() { try { const locked = await lockInspection(currentInspection.id); if (locked.status === 'locked' && locked.lockedAt) { showToast('Baseline secured. It is now read-only.'); navigate(`/inspections/${currentInspection.id}/locked`) } } catch (lockError) { showToast(lockError instanceof Error ? lockError.message : 'The baseline could not be locked.', 'error') } }
