...
export default function JointVerificationPage() {
  ...
  const state = useMemo(() => { if (!inspection) return { ownerAcknowledged: false, renterAcknowledged: false, bothAcknowledged: false }; const ownerAcknowledged = Boolean(inspection.acknowledgements?.[inspection.ownerId]); const renterAcknowledged = Boolean(inspection.renterId && inspection.acknowledgements?.[inspection.renterId]); return { ownerAcknowledged, renterAcknowledged, bothAcknowledged: ownerAcknowledged && renterAcknowledged } }, [inspection])
  ...
  const otherTimestamp = currentUser.id === currentInspection.ownerId ? currentInspection.acknowledgements?.[currentInspection.renterId ?? ''] : currentInspection.acknowledgements?.[currentInspection.ownerId]
